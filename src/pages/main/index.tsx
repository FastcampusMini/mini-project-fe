...
const Main = () => {
  ...
  } = useInfiniteQuery(['products', accessToken], ({ pageParam = 1 }) => ax.getProducts(accessToken, pageParam), {
  ...
  // 추천 상품 가져오기
  const { isLoading: fetchingRecommends, fetchNextPage: fetchNextRecPage } = useInfiniteQuery(
    ...
    <>
      ...
      <main className='flex flex-col overflow-y-scroll scrollbar-none h-full pb-16 relative' ref={ref}>
        ...
        <div className='flex justify-between px-10 mb-5'>
          <div className=''>
            <h1 className='font-bold text-4xl  pointer-events-none'>Let's Get a Loan!</h1>
            <p className='text-black60  pointer-events-none'>
              대출자격은 더 <strong>넓게!</strong> 금리는 더 <strong>낮게!</strong>
            </p>
            <p className='mt-4 text-lg text-black60 font-semibold  pointer-events-none'>
              {userInfo?.name ? userInfo?.name : '(익명)'}님 대출 가능한 금액 :{' '}
              <strong className='text-black80'>{userInfo?.availableAmount <= 0 ? 0 : userInfo?.availableAmount}</strong>
              만 원
            </p>
          </div>
          <div className='space-y-2 flex flex-col items-end'>
            <span className='text-black40 text-sm font-semibold pointer-events-none'>{userInfo?.email}</span>
        ...
        <h3 className='my-3 font-bold text-2xl mx-10  pointer-events-none'>추천상품</h3>
        <div className='flex flex-col gap-5'>
          {!!recommendedProducts.length && <Slider products={recommendedProducts} fetchNextPage={fetchNextRecPage} />}
