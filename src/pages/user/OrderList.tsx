...
import { useDeleteOrderListMutation, useGetOrderListQuery } from '@/store/api/orderApiSlice';
  ...
  return (
    <>
      <article className='h-full px-10 pt-8 pb-24 overflow-y-scroll scrollbar-none'>
        ...
        <h1 className='mb-5 pb-3 text-center text-2xl font-bold border-b border-black'>신청 내역</h1>
        <div className='max-w-screen-sm h-fit'>
          {order?.data?.map((value) => (
            <OrderListElement orderData={value} key={value.orderId} deleteOrderList={deleteOrderList} />
          ))}
          {order?.data === null ? (
            <EmptyCart>
              <AiOutlineFileSearch className='text-7xl' />
              <p className='text-center font-extrabold text-lg'>신청내역이 없습니다.</p>
            </EmptyCart>
          ) : null}
